...
namespace JackFrame {
    ...
    public static class RigidBody2DExtention {
        ...
        public static void MoveInPlatform(this Rigidbody2D rig, float xAxis, float deltaTime, float moveSpeed = 10.5f, float accelerate = 0.018f, float decelerate = 0.0032f, float tentToSpeed = 0) {
            ...
            if (xAxis != 0) {
                ...
                rig.velocity = new Vector2(Mathf.SmoothDamp(rig.velocity.x, moveSpeed * deltaTime * 60 * xAxis.ToOne(), ref xVec, accelerate), rig.velocity.y);

            } else {

                rig.velocity = new Vector2(Mathf.SmoothDamp(rig.velocity.x, tentToSpeed * deltaTime * 60, ref xVec, decelerate), rig.velocity.y);
        ...
        public static void Falling(this Rigidbody2D rig, float jumpAxis, float fixedDeltaTime, float maxFallingSpeed, float gravity = -16f, float fallMultiplier = 4.5f, float raiseJumpMultiplier = 2.5f) {

            // 下落
            if (rig.velocity.y <= 0) {

                rig.velocity += Vector2.up * gravity * fallMultiplier * fixedDeltaTime;
            ...
            // 上升时 托底
                ...
                rig.velocity += Vector2.up * gravity * raiseJumpMultiplier * fixedDeltaTime;
