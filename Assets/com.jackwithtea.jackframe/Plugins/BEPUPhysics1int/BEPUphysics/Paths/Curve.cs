...
namespace BEPUPhysics1int.Paths
{
    ...
    public abstract class Curve<TValue> : Path<TValue>
    {
        ...
        public static Fixed64 ModifyTime(Fixed64 time, Fixed64 intervalBegin, Fixed64 intervalEnd, CurveEndpointBehavior preLoop, CurveEndpointBehavior postLoop)
        {
            if (time < intervalBegin)
            {
                switch (preLoop)
                {
                    case CurveEndpointBehavior.Wrap:
                        Fixed64 modifiedTime = time - intervalBegin;
                        Fixed64 intervalLength = intervalEnd - intervalBegin;
            ...
            else if (time >= intervalEnd)
            {
                ...
                {
                    case CurveEndpointBehavior.Wrap:
                        Fixed64 modifiedTime = time - intervalEnd;
                        Fixed64 intervalLength = intervalEnd - intervalBegin;
                        modifiedTime %= intervalLength;
        ...
        public abstract void Evaluate(int controlPointIndex, Fixed64 weight, out TValue value);
        ...
        public void GetCurveBoundsInformation(out Fixed64 firstIndexTime, out Fixed64 lastIndexTime, out int minIndex, out int maxIndex)
        {
        ...
        public int GetPreviousIndex(Fixed64 time)
        {
            int indexMin = 0;
