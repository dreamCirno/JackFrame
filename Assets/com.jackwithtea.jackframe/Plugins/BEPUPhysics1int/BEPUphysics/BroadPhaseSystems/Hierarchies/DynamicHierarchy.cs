...
namespace BEPUphysics.BroadPhaseSystems.Hierarchies
{
    ...
    public class DynamicHierarchy : BroadPhase
    {
        ...
        public override void Add(BroadPhaseEntry entry)
        {
            ...
            FixedV3 offset;
            FixedV3.Subtract(ref entry.boundingBox.Max, ref entry.boundingBox.Min, out offset);
            if (Fixed64.SafeMul(Fixed64.SafeMul(offset.X, offset.Y), offset.Z) == F64.C0)
            ...
            else
            {
                ...
                else
                {
                    ...
                    FixedV3.Subtract(ref root.BoundingBox.Max, ref root.BoundingBox.Min, out offset);
                    internalNode.currentVolume = Fixed64.SafeMul(Fixed64.SafeMul(offset.X, offset.Y), offset.Z);
        ...
        public Fixed64 MeasureCostMetric()
        {
            if (root != null)
            {
                var offset = root.BoundingBox.Max - root.BoundingBox.Min;
                var volume = Fixed64.SafeMul(Fixed64.SafeMul(offset.X, offset.Y), offset.Z);
                if (volume < F64.C1em9)
                    return F64.C0;
